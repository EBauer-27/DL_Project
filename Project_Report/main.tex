...
\begin{document}
	...
	\todo{citation for Binary cross-entropy loss}
	Both the image-only as well as the multimodal model were trained using Binary Cross-entropy loss (BCEWithLogitsLoss), as it is a standard choice for modeling and evaluating predicted probabilities in binary classification tasks. A weight of positive examples was applied to address class imbalance.
	...
	Further, we performed a hyperparameter optimization using Optuna, to determine optimal hyperparameter values for batch size, learning rate and weight decay. The selected hyperparameters are reported in Table~\ref{tab:Hyperopt}.
	...
	Table~\ref{tab:midas_results} summarizes the performance of the three implemented models. On the MIDAS datset, the TabTransformer achieves the best overall performance, with an AUC of $92\%$, indicating that the metadata contains highly informative features for the lesion classification. 
	...
	In Figure~\ref{gradcam_comparison_numbers} we present a more analytical explanation of the influence of the metadata on the models and how they shift the predictions in the two image based models.
	...
	\section{Conclusion}
	% ============================================================
	As our results show, including patient metadata in the decision-making process of a deep classification model improves performance. Especially in cases where the lesion is not the primary focus, metadata can be a determining factor in correctly classifying the lesion. 
...
%	\end{table}
	...
	\begin{table}[h]
		...
		\caption{Comparison of ResNet image-only and multimodal classification performance for benign/malignant skin lesion diagnosis.}
			...
			Unchanged  & 216 & 78.55\% \\
		\end{tabular}
		\label{tab:resnet_multimodal_comparison}
	\end{table}
	...
	\begin{figure}[H]
		...
		\caption{Both model get distracted by the background and not focus on the lesion. The image-only model predicts a malignant with a nearly $50\%$ confidence, while the multimodal model correctly predicts an benign case due to the metadata contribution.}
